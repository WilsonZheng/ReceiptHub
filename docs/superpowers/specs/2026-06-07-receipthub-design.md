...
- 拍照来源：`<input type="file" accept="image/*,application/pdf" capture="environment">`（iOS Safari 最可靠的调起方式）
...
1. **Capture（默认屏）**：大拍照按钮 → 照片预览（可加拍）→ 五字段表单（日期默认今天；商家输入框带历史自动补全；金额数字键盘；分类 chips 单选；备注可选）→ 保存。公司空间下金额输入时实时显示 GST 金额。目标：一张票 10 秒内录完。
...
- 候选视觉方向（三选一，浏览器 mockup 评审后把结论记回本节）：
